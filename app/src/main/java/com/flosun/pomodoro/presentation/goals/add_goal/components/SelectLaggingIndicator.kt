...
fun SelectLaggingIndicator(
    ...
    var expandedSelectLaggingIndicator by remember { mutableStateOf(false) }

    val label = selectedLaggingIndicator?.let { selectedLaggingIndicator.name }
        ?: "Select Lagging Indicator"
    ...
    Box(
        modifier = Modifier
            ...
            .rippleEffectClickable { expandedSelectLaggingIndicator = true }
            .padding(12.dp),
        ...
        DropdownMenu(
            expanded = expandedSelectLaggingIndicator,
            onDismissRequest = { expandedSelectLaggingIndicator = false },
            ...
            laggingIndicators.forEach { indicator ->
                DropdownMenuItem(
                    modifier = Modifier.fillMaxWidth(),
                    text = {
                        Text(
                    ...
                    onClick = {
                        onChangedSelectedLaggingIndicator(indicator)
                        expandedSelectLaggingIndicator = false
                    }
                )
            }
        }
