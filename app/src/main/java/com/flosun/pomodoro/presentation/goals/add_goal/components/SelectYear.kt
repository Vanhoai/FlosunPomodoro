...
@Composable
fun SelectYear(
    selectedYear: TwelveWeekYearEntity? = null,
    onChangedSelectedYear: (TwelveWeekYearEntity) -> Unit = {},
) {
    val database = LocalDatabase.current
    val screenWidth = ViewFuncs.screenWidthDp()

    val years by database.findAllTwelveWeekYears().collectAsState(initial = emptyList())
    ...
    Box(
        ...
        Text(
            text = selectedYear?.name ?: "Select Year",
            fontSize = 16.sp,
        ...
        DropdownMenu(
            ...
            years.forEach { year ->
                DropdownMenuItem(
                    ...
                    onClick = {
                        onChangedSelectedYear(year)
                        expandedSelectYear = false
                    }
                )
            }
        }
