// It's typically the content of the Composable shader is applied to.
uniform shader uContent;

// The resolution (width, height) of the area where the shader is applied, in pixels.
// Used for accurate cooridnate calculations.
uniform float2 uResolution;

// The current animation time, in seconds. This value advances each frame, driving the
// wave's propagation and damping.
uniform float uTime;
...
// The actual number of waves currently being processed (<= MAX_WAVES).
uniform int uNumWaves;

// Arrays holding the parameters for each active wave.
// These are populated by the Kotlin code.
uniform float2 uWaveOrigins[MAX_WAVES];     // The (x, y) coordinates of each wave's origin point.
uniform float uWaveAmplitudes[MAX_WAVES];   // The initial amplitude (height) of each wave.
...
uniform float uWaveStartTimes[MAX_WAVES];   // The time (in seconds) when each wave was initiated, used to calculate elapsed time for damping.
...
half4 main(float2 fragmentCoordinate) {
    float2 point = fragmentCoordinate;
    ...
    return uContent.eval(fragmentCoordinate + totalOffset);
}